...
# ================= НАСТРОЙКИ =================
BOT_TOKEN = os.getenv("BOT_TOKEN")  # добавь переменную в Railway
ADMIN_IDS = [7750512181]            # твой Telegram ID
FILE_PATH = "files/signed.dll"      # путь к файлу
SCREENSHOT_FOLDER = "files/screens" # путь к скриншотам
...
def callback(call):
    ...
    if call.data == "use_key":
        ...
        bot.send_message(
            ...
            "После оплаты бот автоматически отправит файл.",
            parse_mode="Markdown"
        )
...
def addkey(message):
    ...
    try:
        ...
        KEYS[key] = {"used": False, "multi": mode=="multi"}
        bot.send_message(message.chat.id, f"✅ Ключ `{key}` добавлен", parse_mode="Markdown")
        log(message.from_user, "ADD_KEY", key)
    except:
        bot.send_message(message.chat.id, "❌ Формат: /addkey КЛЮЧ one|multi")
...
def keys(message):
    ...
    for k,v in KEYS.items():
        text += f"{k} — {'многоразовый' if v['multi'] else 'одноразовый'} | {'использован' if v['used'] else 'активен'}\n"
